...
The documentation site follows the [llms.txt](https://llmstxt.org) v2 convention.

- [llms.txt](https://bug3.github.io/aathena/llms.txt) - routing index; every page listed with a one-line note
- Every page has a Markdown twin at the same URL with `.md` instead of `.html`, advertised on the page as `rel="alternate" type="text/markdown"`

[`llms.txt`](llms.txt) in this repository is the same file, generated by `npm run docs:build`. Edit the pages under `docs/`, not it.

[llms-full.txt](llms-full.txt) is a packed dump of the whole repository, regenerated on every push. It is not part of the llms.txt specification and is not published to npm.
