...
import { pascalCase, camelCase, safeIdentifier, isReservedWord } from './utils';
...
export function buildBarrelContent(
  sqlFiles: BarrelInput[],
  ...
  // Detect duplicate query names across tables (based on the raw camelCase
  // output; reserved-word collisions get suffixed independently below).
  ...
  for (const file of sqlFiles) {
    const rawName = camelCase(file.queryName);
    const queryFnName = safeIdentifier(rawName);
    const importPath = `./queries/${file.relativeDirFromTables}/${file.queryName}`;
    // Alias when the name collides with another query OR is a reserved word.
    // Reserved words would otherwise surface as `default_` in user code; the
    // alias replaces that with a nicer `{table}{Query}` form.
    const needsAlias =
      (nameCount.get(rawName) ?? 0) > 1 || isReservedWord(rawName);
...
function generateBarrelIndex(
  ...
  writeFileSync(
    ...
    buildBarrelContent(sqlFiles, schemas),
    'utf-8',
  );
}
