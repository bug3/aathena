...
export async function runAdd(
  ...
  // the scaffold even when --from-schema was not passed.
  try {
    requiredPartitions = await fetchRequiredPartitions(
      resolveRegion(config.region),
      database,
      tableName,
    );
  ...
  const contents = buildQuerySql(tableName, columns, requiredPartitions);
