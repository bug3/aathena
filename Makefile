...
#   immutable tags      a `refs/tags/v*` repository ruleset refuses deletion and
#                       force-push, with no bypass. A published version's
#                       provenance attestation names one commit, so a tag that
#                       has shipped must keep pointing at it. Repair a bad
#                       release by cutting the next one, never by moving a tag.
